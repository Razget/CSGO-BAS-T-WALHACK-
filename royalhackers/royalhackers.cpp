	...
	while (true) {
		if (GLOW == true) {
			for (int i = 0; i < 32; i++) {
				...
				if (ProcessMemory::WPM<float>(R / 27, (glowObject + ((glowIndex * 0x38) + 0x4))) != 0) {
					ProcessMemory::WPM<float>(G / 174, (glowObject + ((glowIndex * 0x38) + 0x8)));
					ProcessMemory::WPM<float>(B / 196, (glowObject + ((glowIndex * 0x38) + 0xC)));
					ProcessMemory::WPM<float>(A / 30, (glowObject + ((glowIndex * 0x38) + 0x10)));
...
int main()
{
	...
	std::cout << "Getting client.dll address!" << std::endl << std::endl;
	CLIENT_BASE = ProcessMemory::FindModule(L"client.dll");
